...
const Pagination = (pages, setCurrentPage, showing, quantityEmployees) => {

    const numOfPages = [];

    for (let i=1; i <= pages.page; i++) {
        numOfPages.push(i)
    }
    ...
    useEffect(() => {
        pages.setCurrentPage(currentButton)
    }, [currentButton, setCurrentPage])

    return (
        <div className="clearfix">
            <div className="hint-text">Showing <b>{pages.showing}</b> out of <b>{pages.quantityEmployees}</b> entries</div>
